...
"""
Model for protein quality control (PQC):
Abeta aggregation dynamics under impaired PQC. 
Sigmoidal transition from a monomer-dominated state to an aggregate-dominated state. 

Initially, efficient proteostasis/transport clearance keeps monomer levels low. 
Aggregates accumulate -> progressively impair clearance through a Hill-type inhibition term -> monomer concentrations to rise. 
This creates a positive feedback loop that accelerates aggregation until the system approaches where aggregate growth dominates -> monomer levels stabilize. 

Distribution is characterized by:
    Early: low aggregate 
    Intermediate: a nonlinear transition phase 
    Late: aggregate accumulation driven by proteostasis collapse 
